...
const SCORES_COLLECTION = "dinoKingdomScores";

setLogLevel("debug");
console.info("[Dino Kingdom Firebase] Initialized", {
  projectId: firebaseConfig.projectId,
  collection: SCORES_COLLECTION
});

if (firebaseConfig.apiKey.includes("YOUR_") || firebaseConfig.appId.includes("YOUR_")) {
  console.warn("[Dino Kingdom Firebase] Firebase config still contains placeholder values", {
...
window.addEventListener("unhandledrejection", (event) => {
  console.error("[Dino Kingdom Firebase] Unhandled promise rejection", event.reason);
});
...
const ICONS = {
  ...
  tigerAdult: "assets/icons/velociraptor-adult-neon.png",
  tigerBaby: "assets/icons/velociraptor-baby-neon.png"
};
...
const CHART_SERIES = {
  grass: { color: "#4f9a4b", visible: true },
  antelope: { color: "#8ea856", visible: true },
  tiger: { color: "#d96a32", visible: true }
};
const SCORE_COMMENTS = [
  {
    ...
    comments: [
      "Life, uh, found a way.",
      "Hammond would have spared no expense.",
      "A park this stable deserves two locked gates.",
      "Even the chaos mathematician is impressed."
    ]
  },
  {
    minDays: 100,
    comments: [
      "Clever girl.",
      "The paddocks held longer than expected.",
      "Grant is cautiously impressed.",
      "Nobody needs to reach for the flare yet."
    ]
  },
  {
    minDays: 50,
    comments: [
      "You did move in herds, briefly.",
      "The fences flickered but held for a while.",
      "The mosquito-in-amber board has concerns.",
      "A decent run before the power went out."
    ]
  },
  {
    minDays: 25,
    comments: [
      ...
      "The raptor paddock has filed a complaint.",
      "This is why Nedry should not run operations.",
    ]
  },
  {
    minDays: 10,
    comments: [
      "The goat lasted longer.",
      "Technically a park, if the map is generous.",
      "The tour vehicles are not returning.",
    ]
  },
  {
    minDays: 0,
    comments: [
      "Must go faster.",
      "The kitchen scene ended badly.",
      "The electric fences were more of a suggestion.",
      "Welcome to Jurassic paperwork."
    ]
  }
];
const RANK_AWARDS = ["🥇", "🥈", "🥉"];
const SOUNDTRACK_FILES = {
  grass: "ferns.mp3",
  antelope: "brontosaurus.mp3",
  tiger: "velociraptor.mp3"
};
const SOUNDTRACK_MAX_VOLUME = 0.36;
const SOUNDTRACK_MIN_VOLUME = 0.02;
const TIGER_TUMMY = 40;
const ANTELOPE_TUMMY = 90;
const CHALLENGE_SHARE_URL = "https://potatocorp.ai/dino-kingdom/?utm=challenge-your-friends";
...
function makeAudioContext() {
  ...
  if (!AudioContextCtor) {
    console.warn("[Dino Kingdom Soundtrack] Web Audio is not supported in this browser");
    return null;
  }
...
async function resumeAudioContextForGesture() {
  ...
  console.info("[Dino Kingdom Soundtrack] AudioContext state", {
    state: context.state
  });
  return context;
}
...
function stopSoundtrackSources() {
  Object.values(soundtrackSources).forEach((source) => {
    try {
      ...
      console.debug("[Dino Kingdom Soundtrack] Source already stopped", error);
    }
  });
  soundtrackSources = {};
  soundtrackGains = {};
...
async function startSoundtrack(mode) {
  ...
  try {
    ...
    console.info("[Dino Kingdom Soundtrack] Started", { mode });
  } catch (error) {
    soundtrackMode = "silent";
    console.warn("[Dino Kingdom Soundtrack] Unable to start", error);
  }
}
...
function setAudioEnabled(enabled) {
  ...
  localStorage.setItem("dinoKingdomAudioEnabled", String(audioEnabled));
  updateAudioToggle();
...
async function shareChallenge() {
  ...
  const text = `My dino kingdom lasted ${score.daysLasted} days. Can you do better?`;
  const shareData = {
    title: "Dino Kingdom",
    text,
  ...
  if (navigator.share) {
    try {
      ...
      console.warn("[Dino Kingdom Share] Native share failed", error);
    }
  }
  ...
  try {
    ...
    console.warn("[Dino Kingdom Share] Unable to share or copy challenge", error);
  }
}
...
function trackGameStarted() {
  trackAnalyticsEvent("dino_kingdom_game_started", {
...
function trackGameEnded(reason) {
  ...
  gameEndTracked = true;
  const score = currentScore();
  trackAnalyticsEvent("dino_kingdom_game_ended", {
    reason,
    days_lasted: score.daysLasted,
    dinos_ever_lived: score.animalsEverLived,
    player_name: playerName || "Anonymous"
  });
}
...
async function loadIntroStats() {
  ...
  try {
    ...
    introStats.textContent = `Kingdoms created: ${entries.length.toLocaleString()} - Dinos expired: ${animalsExpired.toLocaleString()} - Average Kingdom lifespan: ${formattedAverageDays} days`;
  } catch (error) {
    console.warn("[Dino Kingdom Firebase] Intro stats unavailable", {
...
async function submitScoreOnce() {
  if (scoreSubmitted) {
    console.info("[Dino Kingdom Firebase] Score submit skipped: already submitted");
    return;
  }
  ...
  console.info("[Dino Kingdom Firebase] Submitting score", {
    collection: SCORES_COLLECTION,
    score: {
      ...scoreDoc,
      endedAt: "serverTimestamp()"
  ...
  try {
    ...
    scoreSubmitted = true;
    console.info("[Dino Kingdom Firebase] Score submitted", {
      collection: SCORES_COLLECTION,
      documentId: docRef.id
    });
  } catch (error) {
    console.error("[Dino Kingdom Firebase] Unable to submit score", {
      code: error.code,
      message: error.message,
      error
    });
...
async function loadLeaderboard(targetBody = leaderboardBody) {
  console.info("[Dino Kingdom Firebase] Loading leaderboard", {
    collection: SCORES_COLLECTION
  });
  ...
  try {
    ...
    console.info("[Dino Kingdom Firebase] Leaderboard loaded", {
      totalDocs: entries.length,
      displayedDocs: rankedEntries.length
    });
    displayLeaderboard(rankedEntries, targetBody);
  } catch (error) {
    console.error("[Dino Kingdom Firebase] Unable to load leaderboard", {
      code: error.code,
      message: error.message,
      error
    });
...
async function loadInitialLeaderboardMilestones() {
  console.info("[Dino Kingdom Firebase] Loading initial milestone thresholds");
  ...
  try {
    ...
    console.info("[Dino Kingdom Firebase] Initial milestone thresholds loaded", {
    ...
    console.error("[Dino Kingdom Firebase] Unable to load initial milestone thresholds", {
      code: error.code,
      message: error.message,
      error
    });
...
function leaderboardDisplayName(entry) {
  const cake = normalizePlayerName(entry.name) === "marcel" ? " 🎂" : "";
  const label = entry.currentLabel ? ` (${entry.currentLabel})` : "";
  return `${entry.name}${cake}${label}`;
}
...
function displayLeaderboard(entries, targetBody = leaderboardBody) {
  ...
  entries.forEach((entry, index) => {
    ...
    name.textContent = leaderboardDisplayName(entry);
...
function showLeaderboardPanel() {
  console.info("[Dino Kingdom Firebase] Showing leaderboard panel");
  playerNamePanel.classList.add("is-hidden");
...
function showLeaderboardViewer() {
  console.info("[Dino Kingdom Firebase] Showing persistent leaderboard viewer");
...
function pauseForBiodiversityLoss() {
  ...
  console.info("[Dino Kingdom Firebase] Biodiversity-loss game end reached", currentScore());
  ...
  pauseButton.textContent = "■";
  simStatus.textContent = "Dinos Gone";
  gameOverTitle.textContent = "Kingdom fallen - dinos extinct";
...
function showTerminalGameOver() {
  renderFinalFrameBeforeGameOver();
  console.info("[Dino Kingdom Firebase] Terminal game end reached", currentScore());
...
function render() {
  ...
  document.getElementById("antelopeDeaths").textContent =
    `hatched ${deathStats.antelope.born} · hunted ${deathStats.antelope.eaten} · old ${deathStats.antelope.oldAge} · starved ${deathStats.antelope.starvation}`;
  document.getElementById("tigerDeaths").textContent =
    `hatched ${deathStats.tiger.born} · old ${deathStats.tiger.oldAge} · starved ${deathStats.tiger.starvation}`;
  drawChart();
}
...
function bindEvents() {
  Object.entries(controls).forEach(([key, input]) => {
    if (!input) {
      console.warn(`[Dino Kingdom] Missing control input: ${key}`);
      return;
    }
  ...
  resetButton.addEventListener("click", (event) => {
    ...
    console.info("[Dino Kingdom] Reset requested from toolbar");
  ...
  startButton.addEventListener("click", () => {
    ...
    localStorage.setItem("dinoKingdomPlayerName", playerName);
    resetSimulation();
  });
...
function showWelcomeOverlay() {
  ...
  gameOverTitle.textContent = "Begin your dino kingdom";
  gameOverMessage.textContent = "Set up your kingdom with the sliders. Start the Jurassic ecosystem when you are ready.";
  continueButton.classList.add("is-hidden");
  challengeButton.classList.add("is-hidden");
  startButton.textContent = "Start";
  playerNameInput.value = localStorage.getItem("dinoKingdomPlayerName") || "";
...
function showResetSetupOverlay() {
  ...
  soundtrackMode = "pregame";
  updateSoundtrackMix(false);
  gameOverTitle.textContent = "Reset your dino kingdom";
  gameOverMessage.textContent = "Change the settings, then begin again when you are ready.";
  continueButton.classList.add("is-hidden");
  challengeButton.classList.add("is-hidden");
  startButton.textContent = "Begin Again";
  playerNameInput.value = playerName || localStorage.getItem("dinoKingdomPlayerName") || "";
  playerNamePanel.classList.add("is-hidden");
  leaderboardPanel.classList.add("is-hidden");
  startOverlay.classList.remove("is-hidden");
}
